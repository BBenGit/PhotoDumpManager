...
def get_file_name(create_datetime, extension, add_microsecond):
    filename = str(create_datetime.date()) + "_" + \
        str(create_datetime.time()).replace(":", "")
...
def get_file_tags(path):
    with open(path, 'rb') as file:
        try:
            ...
            logger.error(
                "TypeError while getting files tags for file {}".format(path))
            tags = {}
...
def get_tag(tags, tag_name):
    ...
    try:
        ...
        logger.error(
            "KeyError for '{}'. Tags list = {}".format(tag_name, tags))
    finally:
        return string


def get_image_create_datetime(path):
    datetime_original = get_cmd_output(
        'exiftool -DateTimeOriginal "' + path + '"', "Date/Time Original")
    create_date = get_cmd_output(
        'exiftool -CreateDate "' + path + '"', "Create Date")
...
def get_cmd_output(cmd, string_prefix):
    ...
    result = str(output).replace("b'" + string_prefix,
                                 "").replace("'", "").replace("\\n", "")
        ...
        if not filename.startswith('.'):  # Ignore hidden files
            if os.path.isfile(os.path.join(inner_directory, filename)):
                with open(os.path.join(inner_directory, filename), 'rb') as file:
                    ...
                    # Remove trailing point between filename and extension
                    extension = Path(filename).suffix[1:]

                    do_sorting = False

                    if extension.lower() in image_types:
                        access_time = get_image_create_datetime(
                            os.path.join(inner_directory, filename))
                        do_sorting = True
                    elif extension.lower() in video_types:
                        access_time = get_video_create_datetime(
                            os.path.join(inner_directory, filename))
                        do_sorting = True
                    else:
                ...
                if recursive & (os.path.join(inner_directory, filename) != output_directory):
                    sort(os.path.join(inner_directory, filename),
                         output_directory, recursive)
    ...
    for filename in os.listdir(inner_directory):
        if not filename.startswith('.'):
            if os.path.isfile(os.path.join(inner_directory, filename)):
                ...
                update_timezone_metadata(os.path.join(
                    inner_directory, filename), timezone, timezone_sign)
...
def check_directories(input_directory, output_directory):
    ...
    if not os.path.exists(output_directory):
        logger.info("Missing output directory at %s. Creating it…",
                    output_directory)
    ...
    for filename in os.listdir(input_directory):
        if not filename.startswith('.'):
            if os.path.isfile(os.path.join(input_directory, filename)):
                ...
                shift_all_date(os.path.join(input_directory,
                                            filename), shift_sign, shift_string)
...
if __name__ == "__main__":
    ...
    if choice == "s":
        ...
        date = datetime.fromisoformat(
            input("Select date to rename file (YYYY-MM-DDTHH:MM:SS): "))
        ...
        input_directory = input(
            "Select directory of your files you want to change timezone metadata: ")
        ...
        input_directory = input(
            "Select directory of your files you want to shift time metadata: ")
